...
def compute_traction_components_nodal(
    ...
    Pn0 = np.zeros(n_vertex)
    Ts0 = np.zeros(n_vertex)
    Td0 = np.zeros(n_vertex)
    ...
    for k, node in enumerate(selected_vertex):
        ...
        Pn0[k] = np.sum(tractions.T * una, axis=0)
        Ts0[k] = np.sum(tractions.T * usa, axis=0)
        Td0[k] = np.sum(tractions.T * uda, axis=0)
    out = {}
    out["T_n"] = Pn0
    out["T_s"] = Ts0
    out["T_d"] = Td0
...
def gridto2Dlocal(
...
) -> Tuple[Grid2D, Dict[str, np.ndarray]]:
    """
    ...
    Returns:
    Tuple[Grid2D, Dict[str, np.ndarray]]: Gridded 2D local coordinate system and corresponding data.
    """
    ...
    for dataName, myData in traction_components.items():
        ...
        if plot_data:
            ...
            plt.axis("equal")
            plt.show()
        lgridded_myData[dataName] = gridded_myData
...
def writeAllNetcdf(
    mygrid: Grid2D,
    lgridded_myData: Dict[str, np.ndarray],
    sName: str,
    ...
    if paraview_readable:
        for key, value in lgridded_myData.items():
            writeNetcdf(
                f"{sName}_{key}",
                [mygrid.u, mygrid.v],
                [key],
    ...
    writeNetcdf(
        f"{sName}_TsTdTn",
        [mygrid.u, mygrid.v],
        list(lgridded_myData.keys()),
        list(lgridded_myData.values()),
...
def generate_input_files(
    ...
    for itag, tag in enumerate(unique_tags):
        ...
        if elemental:
            tractions = compute_tractions(stress, un[:, ids])
            traction_components = compute_traction_components(
                tractions, us[:, ids], ud[:, ids], un[:, ids]
            )
        else:
        ...
        vertex_selected = faultCentroids[ids] if elemental else xyz_selected
        ...
        parameters = ",".join(traction_components.keys())
